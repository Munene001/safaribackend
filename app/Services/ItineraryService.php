...
class ItineraryService
{
        ...
        return DB::transaction(function () use ($data) {
            ...
            foreach ($data['sub_itineraries'] as $subData) {
                ...
                foreach ($subData['day_plans'] as $dayPlanData) {
                    ...
                    if (!empty($dayPlanData['accommodation_id'])) {
                        $subItinerary->accommodations()->attach(
                            $dayPlanData['accommodation_id'],
    ...
    {
        return DB::transaction(function () use ($itineraryId, $subItineraryId, $data) {
            $subItinerary = SubItinerary::where('itinerary_id', $itineraryId)
                ->findOrFail($subItineraryId);

            // Check for existing attachment to prevent duplicates
            $existing = $subItinerary->accommodations()
                ->where('Sub_Itinerary_Accommodations.accommodation_id', $data['accommodation_id'])
                ->exists();

            if ($existing) {
                throw new \Exception('Accommodation already attached for this night');
            }
            ...
            return $subItinerary->load('accommodations');
        });
    }
}
